...
class OpenShiftBuildDockerfile extends BuildStep {
  ...
  async build(version: VersionDoc, _files: Files, state: any): Promise<void> {
    ...
    const { namespace, dockerPushSecret } = config.build.openshift
    const registryUrl = `${config.app.host}:${config.app.port}`

    try {
      await client.api.v1.ns(namespace).secret(dockerPushSecret).get()
